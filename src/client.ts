...
export interface ClientOptions {
  /**
   * Defaults to process.env['HANDINGER_API_KEY'].
   */
   ...
   *
   * Defaults to process.env['HANDINGER_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['HANDINGER_LOG'] or 'warn' if it isn't set.
   */
...
export class Handinger {
   ...
   */
  constructor({
    baseURL = readEnv('HANDINGER_BASE_URL'),
    apiKey = readEnv('HANDINGER_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.HandingerError(
        "The HANDINGER_API_KEY environment variable is missing or empty; either provide it, or instantiate the Handinger client with an apiKey option, like new Handinger({ apiKey: 'My API Key' }).",
      );
    }
    ...
    this.timeout = options.timeout ?? Handinger.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('HANDINGER_LOG'), "process.env['HANDINGER_LOG']", this) ??
      defaultLogLevel;
  ...
  static Handinger = this;
  static DEFAULT_TIMEOUT = 60000; // 1 minute

  static HandingerError = Errors.HandingerError;
