...
import { ClientOptions } from '@ramensoft/handinger';

const prompt = `Runs JavaScript code to interact with the Handinger API.
...
const remoteStainlessHandler = async ({
  ...
  const localClientEnvs = {
    HANDINGER_API_KEY: requireValue(
      readEnv('HANDINGER_API_KEY') ?? client.apiKey,
      'set HANDINGER_API_KEY environment variable or provide apiKey client option',
    ),
    HANDINGER_BASE_URL: readEnv('HANDINGER_BASE_URL') ?? client.baseURL ?? undefined,
  };
...
const localDenoHandler = async ({
  ...
  try {
    const sdkPkgName = '@ramensoft/handinger';
